...
const API_BASE_URL = 'http://localhost/api-sqlserver/peliculas.php';
...
const listar_peliculas = () => {
    return fetch(API_BASE_URL)
        ...
        .catch((error) => {
            console.error('Error al obtener las películas desde SQL Server:', error);
            throw error;
        });
};
...
const crear_pelicula = (titulo, duracion, clasificacion, imagen_url) => {
    return fetch(API_BASE_URL, {
        ...
        if (!response.ok) {
            throw new Error('Error al crear la película en SQL Server');
        }
        return response.json();
    });
};
...
const eliminar_pelicula = (id) => {
    return fetch(`${API_BASE_URL}?id=${id}`, {
        ...
        if (!response.ok) {
            throw new Error('Error al eliminar la película en SQL Server');
        }
        return response.json();
    });
};
...
const obtener_pelicula = (id) => {
    return fetch(`${API_BASE_URL}?id=${id}`)
        .then((respuesta) => {
            if (!respuesta.ok) {
                throw new Error('Error al obtener la película desde SQL Server');
            }
...
const actualizar_pelicula = (id, titulo, duracion, clasificacion, imagen_url) => {
    return fetch(API_BASE_URL, {
        ...
        if (!respuesta.ok) {
            throw new Error('Error al actualizar la película en SQL Server');
        }
        return respuesta.json();
    });
};
