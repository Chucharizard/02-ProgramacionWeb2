...
const API_BASE_URL = 'http://localhost/api-sqlserver/funciones.php';
...
const listar_funciones = () => {
    return fetch(API_BASE_URL)
        ...
        .catch((error) => {
            console.error('Error al obtener las funciones desde SQL Server:', error);
            throw error;
        });
};
...
const crear_funcion = (pelicula_id, sala, fecha, hora, precio) => {
    return fetch(API_BASE_URL, {
        ...
        if (!response.ok) {
            throw new Error('Error al crear la función en SQL Server');
        }
        return response.json();
    });
};
...
const eliminar_funcion = (id) => {
    return fetch(`${API_BASE_URL}?id=${id}`, {
        ...
        if (!response.ok) {
            throw new Error('Error al eliminar la función en SQL Server');
        }
        return response.json();
    });
};
...
const obtener_funcion = (id) => {
    return fetch(`${API_BASE_URL}?id=${id}`)
        .then((respuesta) => {
            if (!respuesta.ok) {
                throw new Error('Error al obtener la función desde SQL Server');
            }
...
const actualizar_funcion = (id, pelicula_id, sala, fecha, hora, precio) => {
    return fetch(API_BASE_URL, {
        ...
        if (!respuesta.ok) {
            throw new Error('Error al actualizar la función en SQL Server');
        }
        return respuesta.json();
    });
};
